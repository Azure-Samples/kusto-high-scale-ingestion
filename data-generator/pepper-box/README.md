...
* **linger.ms** : How much maximum timeDelta producer should wait till batch becomes full(should be 5-10 when increased batch size and compression is enabled)
...
|----------|:-------:|:--------:|:--------:|
|TIMESTAMP()|current timeDelta in long|```{{TIMESTAMP()}}```|Long|
...
**Note:** While writing custom functions, please try to keep data in memory or scale your function as much other functions otherwise your custom function itself becomes performance bottlneck. e.g. you need some record ids from RDBMS for some schema fields, instead of querying every timeDelta bring all ids inmemory and get random id from those ids.

You can also add manipulations on template functions, for example TIMESTAMP() function returns timeDelta in milliseconds but you can get timeDelta in seconds,
